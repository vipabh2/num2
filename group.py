...
async def link(event):
    ...
    try:
        ...
        return await event.reply("🚫 لا يوجد حساب بهذا الآيدي...")
    tag = user.first_name.replace("\u2060", "") if getattr(user, 'first_name', None) else (f"@{user.username}" if user.username else "مستخدم")
    button = [Button.inline("🔗 تغيير إلى رابط", b"recgange")]
    await event.reply(
        f"⌔︙[{tag}](tg://user?id={user.id})",
        buttons=button,
        link_preview=False
    )
